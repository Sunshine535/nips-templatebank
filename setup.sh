#!/usr/bin/env bash
# NeurIPS 2026 — nips-templatebank: conda env, PyTorch 2.4.0 + CUDA 12.1, deps, flash-attn, verify
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
cd "$ROOT"

ENV_NAME="${NIPS_CONDA_ENV:-nips-templatebank}"
PY_VER="${PYTHON_VERSION:-3.10}"

echo "============================================"
echo " nips-templatebank — environment setup"
...
for mod in ("transformers", "peft", "trl", "datasets", "accelerate", "scipy"):
